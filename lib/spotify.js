...
// Client-side helpers for Spotify PKCE auth, token persistence, and API requests.
...
const REDIRECT_URI =
  process.env.NEXT_PUBLIC_SPOTIFY_REDIRECT_URI || DEFAULT_REDIRECT;
...
export async function initiateSpotifyLogin() {
  if (!CLIENT_ID) {
    throw new Error(
      'Missing Spotify client id. Set NEXT_PUBLIC_SPOTIFY_CLIENT_ID in a .env.local file.'
    );
  }
